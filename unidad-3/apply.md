...
[liink p5.js](https://editor.p5js.org/mechats/sketches/IFoeiOsXt)
...
function recibirSerial() {
  let entrada = serial.readLine().trim();
  if (entrada.length > 0) {
    ultimaEntrada = entrada;
    manejarEventoSerial(entrada);
  }
}

function manejarEventoSerial(evento) {
  if (evento === "A") {
    presionarBoton('A');
  } else if (evento === "B") {
    presionarBoton('B');
  } else if (evento === "SHAKE") {
    presionarShake();
  } else if (evento === "RESET") {
    presionarReiniciar();
  }
}
```
